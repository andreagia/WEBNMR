...
This module defines files utilities for virtualcalc.
"""
...
def copy(path, file, destdir, check_fileexists = 1):
    ...
    if os.path.isdir(destdir):
        ...
        if os.path.exists(fulldestdir) and check_fileexists:
            return (False, os.path.basename(fulldestdir))
    else:
        if os.path.exists(destdir) and check_fileexists:
            return (False, os.path.basename(destdir))
        fulldestdir = destdir
    try:
...
def move(path, file, destdir, check_fileexists = 1):
    ...
    if os.path.isdir(destdir):
        fulldestdir = os.path.join(destdir, file)
        if os.path.exists(fulldestdir) and check_fileexists:
            return ('exist', os.path.basename(fulldestdir))
    else:
        if os.path.exists(destdir) and check_fileexists:
            return ('exist',os.path.basename(destdir))
        fulldestdir = destdir
